...
def word_count():
    ...
    content = "Flink Flink pyflink flink pyflink Flink"
    # 设定flink执行环境
    ...
    # 定义临时表Temp1，存放中间结果
    temp_file_1 = "/tmp/table1"
    if os.path.isfile(temp_file_1):
        os.remove(temp_file_1)
    else:
        # 递归删除所有子文件夹和子文件。
        shutil.rmtree(temp_file_1)
    t_env.connect(FileSystem().path(temp_file_1)) \
        .with_format(OldCsv()
                     .field_delimiter(',')
                     .field("word", DataTypes.STRING())
                     .field("count", DataTypes.BIGINT())) \
    ...
    # 定义临时表Temp2，存放中间结果
    temp_file_2 = "/tmp/table2"
    if os.path.isfile(temp_file_2):
        os.remove(temp_file_2)
    else:
        # 递归删除所有子文件夹和子文件。
        shutil.rmtree(temp_file_2)

    t_env.connect(FileSystem().path(temp_file_2)) \
        .with_format(OldCsv()
                     .field_delimiter(',')
                     .field("word", DataTypes.STRING())
                     .field("count", DataTypes.BIGINT())) \
    ...
    # 定义临时表Temp3，存放中间结果
    temp_file_3 = "/tmp/table3"
    if os.path.isfile(temp_file_3):
        os.remove(temp_file_3)
    else:
        # 递归删除所有子文件夹和子文件。
        shutil.rmtree(temp_file_3)
    t_env.connect(FileSystem().path(temp_file_3)) \
        .with_format(OldCsv()
                     .field_delimiter(',')
                     .field("word", DataTypes.STRING())
                     .field("count", DataTypes.BIGINT())) \
