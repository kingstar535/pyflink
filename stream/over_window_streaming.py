...
def over_window_streaming():
    ...
    result_file = os.getcwd() + os.sep + "output" + os.sep + "table_over_window_agg_streaming_004.csv"
    ...
    w1 = Over.partition_by("a") #可以按多个元素进行partition, 多个key使用,分割 
    ...
    # 实时统计每个用户，购买的商品次数和商品的数量。
    # count返回类型为BIGINT
    t3 = t2.select("a, b.count over w as b, c.sum over w as sum")
    t3.print_schema()
